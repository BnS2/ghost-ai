...
2. Add the participant avatar group inside the editor canvas area.
   ...
   - render the current user separately as a static avatar using
     the active Clerk user's profile image, labelled "You" —
     do not render a second avatar for them from the Liveblocks
     presence list
   ...
   - if no collaborators are present, show only the Clerk
     profile avatar with no divider
...
- don't use the Clerk UserButton in the canvas avatar group
