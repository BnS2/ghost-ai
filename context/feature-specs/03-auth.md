...
Define public routes using the existing sign-in and sign-up env vars (handled by varlock through `.env.schema`). Protect everything else by default.

Update `/`:

- authenticated users redirect to `/editor`
- unauthenticated users redirect to `/sign-in`
- already signed in users visiting sign-in/sign-up pages should be redirected to `/editor`

Add Clerk's built-in `UserButton` to the editor navbar right section for profile settings and logout.

Keep Clerk's default user menu and profile flows intact. Do not rebuild or heavily customize Clerk itnernals.

Use existing Clerk env vars (handled by varlock through `.env.schema`). Do not rename or invent new ones.
